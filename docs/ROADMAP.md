...
- **Task 004: 공통 컴포넌트 및 더미 데이터 구축**
  - 추가 shadcn/ui 컴포넌트 도입 (tabs, dialog, textarea, select, avatar, table, tabs 등)
...
- **Task 011: Admin 기능 구현**
  ...
  - Admin 권한 가드 (미들웨어/레이아웃 레벨 role 검증)
  - 더미 통계를 실제 집계 쿼리로 교체
  - **테스트**: Playwright MCP로 Admin 통계 조회·역할 변경·모임 강제 관리 플로우 E2E 테스트 (비-Admin 접근 차단 검증)
