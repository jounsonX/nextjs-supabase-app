...
- **Task 011-1: 핵심 기능 통합 테스트** ✅ - 완료
  - ✅ Playwright MCP를 사용한 전체 사용자 플로우 E2E 테스트
    - ✅ 주최자: 모임 생성(F001) → 공지 작성/핀 고정(F005) → 참여자 승인/거절(F008) → 정산 납부 확인(F009)
    - ✅ 참여자: 모임 탐색(F002) → 참여 신청(F007) → 카풀 등록 및 동승 신청(F010)
    - ✅ 운영자: 통계 대시보드(F013) → 사용자 역할 변경(F014) → 모임 강제 마감/취소(F015)
  - ✅ 비즈니스 로직 경계값 검증
    - ✅ Waitlist 자동 승격 (승인 참여자 거절 → 대기자 자동 approved 전환)
    - ✅ 정산 올림 계산 `Math.ceil(10000/3) = 3,334원` UI 표시 확인
    - ✅ 카풀 선착순 만석 (잔여 0석 → "자리 없음" disabled 버튼)
  - ✅ 에러/엣지 케이스 검증
    - ✅ 비호스트 공지 작성 버튼 미노출 (UI 분기)
    - ✅ 이미 승인된 참여자 중복 신청 방지 ("참여 취소" 버튼만 표시)
    - ✅ 드라이버 본인 카풀 동승 신청 버튼 미노출
    - ✅ 비Admin 계정 `/protected/admin` 접근 거부 UI 확인
  - ✅ 버그 발견 및 수정
    - ✅ `profiles` UPDATE RLS 무한재귀 → `is_admin()` SECURITY DEFINER 함수로 수정
    - ✅ 카풀 등록 버튼 호스트 전용 버그 → 승인된 참여자도 등록 가능하도록 수정 (`myStatus === 'approved'`)
